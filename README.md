
<font size="1">
<span style="color:green">
 Simple_YoutubeAnalytic_Grapher </span>
Takes the youtube API and graphs the values. 
<font size="1">
 This is a simple youtube analytic grapher.
<font size="1">
 To use the youtube API the user is going to need to generate a youtube developer api key . 
<font size="1">
 The app is meant to use a variety of libraries and I m attempting to provide a way for tkinkter to work with altair vega, if that is at all possible. If not, then just use the alt_viewer </font>

