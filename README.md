
<font size="1">
<span style="color:green">
 Simple_YoutubeAnalytic_Grapher </span>
Takes the youtube API and graphs the values. 
<font size="1">
 This is a simple youtube analytic grapher.
<font size="1">
 To use the youtube API the user is going to need to generate a youtube developer api key . This was simple project for me to get some practice with the python language.I mention this because there are objects, liked the pickeled object, that I am not using for anything in this program. 
<font size="1">
</font>

** please note that this will not work unless you provide your API key for the variable and set the q parameter in the build method for the discovery object


<font size = "1" > 
youtubeApiKey= ""  # set this variable to your personal developer API key. 
snippets = youtube.search().list(part="snippet", type="channel", q="").execute() 
# in this line about under the "q" parameter set it to a channel hardcoded or do it dynamically 
